...
/**
 * Hooks for MaamediaMessages extension
 *
 * @file
 * @ingroup Extensions
 */
	...
	SpecialPageBeforeExecuteHook
{
	...
	/**
	 * When core requests certain messages, change the key to a Maamedia version.
	 *
	...
	public function onSkinCopyrightFooter( $title, $type, &$msg, &$link ) {
		...
		if (
			$type !== 'history' &&
			// Only do this on Maamedia wikis that are using CC-BY-SA, i.e. not on Wikinews
			...
			switch ( $dbName ) {
				case 'datawiki':
				case 'testwiki':
					$msg = 'datawiki-copyright';
					break;
				case 'commonswiki':
					$msg = 'maamedia-commons-copyright';
					break;
				default:
					$msg = 'maamedia-copyright';
			}
		}
	}
	...
	public static function onMobileLicenseLink( &$link, $context, array $attribs, &$msg ) {
		...
		switch ( $wgDBname ) {
			case 'datawiki':
			case 'testwiki':
				// Data-Wiki needs its own special message. See T112088
				$msg = 'datawiki-copyright';
			...
			default:
				...
				if ( strpos( $wgRightsUrl, 'creativecommons.org/licenses/by-sa/3.0' ) !== false ) {
					...
					if ( $context === 'editor' || $context === 'talk' ) {
						$link = wfMessage( 'maamedia-mobile-license-links' )
							->inContentLanguage()
							->plain();
					}
				}
	...
	public function onSkinAddFooterLinks( Skin $skin, string $key, array &$footerLinks ) {
		...
		$devDestination = Skin::makeInternalOrExternalUrl(
			$skin->msg( 'maamedia-developers-url' )->inContentLanguage()->text()
		);
		$devLink = Html::element(
			'a',
			[ 'href' => $devDestination ],
			$skin->msg( 'maamedia-developers' )->text()
		);
		$footerLinks['developers'] = $devLink;

		$statsDestination = Skin::makeInternalOrExternalUrl(
			$skin->msg(
				'maamedia-statslink-url', $skin->getConfig()->get( 'ServerName' )
			)->inContentLanguage()->text()
		);
		$statsLink = Html::element(
			'a',
			[ 'href' => $statsDestination ],
			$skin->msg( 'maamedia-statslink' )->text()
		);
		$footerLinks['statslink'] = $statsLink;

		$cookieDestination = Skin::makeInternalOrExternalUrl(
			$skin->msg( 'maamedia-cookiestatement-page' )->inContentLanguage()->text()
		);
	...
	public static function onGlobalBlockingBlockedIpMsg( &$msg ) {
		$msg = 'maamedia-globalblocking-ipblocked';
	}
	...
	public static function onGlobalBlockingBlockedIpRangeMsg( &$msg ) {
		$msg = 'maamedia-globalblocking-ipblocked-range';
	}
	...
	public static function onGlobalBlockingBlockedIpXffMsg( &$msg ) {
		$msg = 'maamedia-globalblocking-ipblocked-xff';
	}
	 ...
	 * then-current Maamedia configuration and contents of LanguageXx.php.
	 */
	...
	public function onSpecialPageBeforeExecute( $special, $subPage ) {
		...
		$link = Html::rawElement(
			'a',
			[
				'href' => 'https://meta.maamedia.org/wiki/Special:MyLanguage/' .
